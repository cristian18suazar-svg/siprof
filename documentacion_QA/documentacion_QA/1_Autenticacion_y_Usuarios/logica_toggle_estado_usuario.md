...
Para realizar esta accion, el usuario hace clic en un boton o switch que apunta a AdminUsuarioController.php?accion=toggleEstado&id=X&estado=Y.
...
El controlador recibe id y estado por GET. Valida que el ID sea mayor a 0. Llama al metodo toggleEstado() del modelo que convierte el valor numerico a texto: si estado es 1 pone Activo, si es 0 pone Inactivo. Ejecuta UPDATE usuario SET Estado igual al nuevo estado WHERE IDusuario igual al id recibido.
